...
int minCost(int cur, vl &v, ml &mp) {
	if(mp.find(cur) != mp.end()) return mp[cur];
	if(cur == v.size() - 1) return 0;
	if(cur == v.size() - 2) return abs(v[v.size()-1] - v[v.size()-2]);
	int opt1 = abs(v[cur] - v[cur+1]) + minCost(cur+1, v, mp);
	int opt2 = abs(v[cur] - v[cur+2]) + minCost(cur+2, v, mp);
	int best = min(opt1, opt2);
	mp.insert({cur, best});
	return mp[cur];
}
