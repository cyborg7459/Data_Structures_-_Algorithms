...
int maxHappiness(int day, int event, int prev, int n, vector<vector<int>> &v, map<pair<int, int>, int> &dp) {
	if(dp.find({day, event}) != dp.end()) return dp[{day, event}];
	if(day > n) return 0;
	if(day == n) return v[day][event];
	int res = 0;
	for(int i=0; i<3; i++) {
		if(i != event)
			res = max(res, v[day][event] + maxHappiness(day+1, i, event, n, v, dp));
	}
	dp.insert({{day, event}, res});
	return res;
}
...
void solve() {
	...
	loop(i, 3) {
		loop(j, 3) {
			if(j != i)
				res = max(res, v[1][i] + maxHappiness(2, j, i, n, v, dp));
		}
	}
	o(res);
}
